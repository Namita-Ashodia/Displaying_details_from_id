...
location.reload(true);
 ...
 <?php
     ...
     // echo ($sql);
     $sql = "SELECT * from lead WHERE assignedto like '%$emp_email%' ";
   ...
   <div class="w3-row-padding w3-responsive w3-margin-bottom">
          <table id="id02" border="1" class="w3-table-all" style='font-size:12px;'>
                <tr class='w3-blue w3-small'>
                  ...
                  <th>Notification Date Remainder</th>
                  <th>your given to </th>
                  <th>your reply</th>
                  <th>your approval</th>
                  <th>your status</th>
                  <th>Reply by Sale executive</th>
                  <th>approval by Sale executive</th>
                  <th>status by Sale executive</th>
                  <th>notification date by sales</th>
                  <th>reply after notification date by sales</th>
...
<?php
  while ($sql_rs = mysqli_fetch_assoc($sql_res)) : ?>
   ...
   <!--  <td><?php echo str_replace(',', '<br>',$sql_rs['giveto']); ?></td>
...
?>
    ...
    -->      <?php
       ...
       $sql1="select * from tms_admin";
       $sql1_fire= mysqli_query($dbc, $sql1) or die(mysqli_error($dbc));
      ...
      $sql_user_res = mysqli_query($dbc, $sql_user) or die(mysqli_error($dbc));
       ...
       // $emp=mysqli_num_rows(sql_user_res);
    ?>
   
    <form action='apanel/processform/leadapproval.php' method="POST">
    <td>
   
   <input type="date" name="lnotificationdate" class="w3-input w3-border" value="<?php echo $lnotificationdate; ?>" >

</td>

     <td><div >
     <label class="w3-text-grey">Assigned to</label>
       <select  id="giveto" name="giveto[]" multiple>
             <option value="" disabled selected>Assigned to</option>
             <?php
            
             while($sql_user_rs = mysqli_fetch_assoc($sql1_fire)) : ?>
               <option value="<?php echo($sql_user_rs['email']);?>" 
                 >
               <?php echo trim($sql_user_rs['fname'])." ".trim($sql_user_rs['lname'])." (".trim($sql_user_rs['email']).")" ; echo $adm; ?></option>
              

          <?php endwhile; 
            
              while($sql_user_rs = mysqli_fetch_assoc($sql_user_res)) : ?>
              <option value="<?php echo($sql_user_rs['email']); ?>" 
                >
              <?php echo trim($sql_user_rs['fname'])." ".trim($sql_user_rs['lname'])." (".trim($sql_user_rs['email']).")" ; ?></option>
...
<input type="text"  id="reply" name="reply" placeholder="reply">
   ...
   <input type="radio" name="approval" value="no"> no<br>
         ...
         <td style='width:50%;'>
         <input type="radio" name="status" value="final">final<br>
        <input type="radio" name="status" value="working on it">working on it<br>
   ...
   <?php  $current_date = date('Y-m-d');
    //echo($current_date) ?>
    <?php  $interval = (strtotime($sql_rs['lnotificationdate']) - strtotime($current_date));  
           $intervals =  ($interval / 86400); ?>
    <?php  
    //if($intervals < 0) {echo "<td style='color:red;font-weight:bold;'>".$intervals.' Days'."</td>"; }
    if($intervals == 0) 
   ...
   <td><?php echo $sql_rs['giveto'];  ?></td>
   <td><?php echo $sql_rs['reply'];  ?></td>
   <td><?php echo $sql_rs['approval'];  ?></td>
   <td><?php echo $sql_rs['status'];  ?></td>
   ...
   <td><?php echo $sql_rs['replycall'];  ?></td>
